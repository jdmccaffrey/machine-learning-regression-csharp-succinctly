...
namespace NearestNeighborsRegressionWeighted
{
  internal class NearestNeighborsRegressionWeightedProgram
  {
    static void Main(string[] args)
    {
      ...
      int numNeighbors = 4;
      string distMetric = "cosine";  // 'euclidean' or 'cosine' supported
      double[] wts = new double[] { 0.40, 0.30, 0.15, 0.15 };
      
      Console.WriteLine("Setting numNeighbors = " + numNeighbors);
      Console.WriteLine("Setting distance metric = " + distMetric);
      Console.Write("Setting neighbor weights = "); VecShow(wts, 4, 9);
      NearestNeighborsRegressor model =
        new NearestNeighborsRegressor(numNeighbors, wts, distMetric);
  ...
  public class NearestNeighborsRegressor
  {
    ...
    public string distanceMetric;  // 'euclidean' or 'cosine'
    public double[][] trainX;
    public double[] trainY;

    // ------------------------------------------------------------------------

    public NearestNeighborsRegressor(int nNeighbors, double[] wts,
      string distanceMetric)
    {
      this.nNeighbors = nNeighbors;
      this.weights = new double[nNeighbors];
      for (int k = 0; k < nNeighbors; ++k)
        this.weights[k] = wts[k];
    ...
    public double Predict(double[] x)
    {
      ...
      for (int i = 0; i < N; ++i)
      {
         dists[i] = this.Distance(x, this.trainX[i]);  // routes to specific
      }

      // 2. determine sort order of distances
      int[] sortedIdxs = ArgSort(dists);

      // 3. weighted average of nearest target y values
      double sum = 0.0;
    ...
    public void Explain(double[] x)
    {
      ...
      for (int i = 0; i < n; ++i)
        distances[i] = this.Distance(x, this.trainX[i]);
    ...
    private double Distance(double[] v1, double[] v2)
    {
      ...
      if (this.distanceMetric == "euclidean")
        ...
        Console.WriteLine("distance metric not recognized, using euclidean ");
      
      return EuclideanDist(v1, v2); // default 
    }
    ...
    private static double CosineDist(double[] v1, double[] v2)
    {
      int n = v1.Length;
      ...
      for (int i = 0; i < n; ++i)
      {
        ...
        sumSquares1 += v1[i] * v1[i];
        sumSquares2 += v2[i] * v2[i];
      }

      double norm1 = Math.Sqrt(sumSquares1);
      double norm2 = Math.Sqrt(sumSquares2);

      double cosineSim = dotProduct / (norm1 * norm2);
      double cosineDist = 1.0 - cosineSim;
      return cosineDist;
    }
